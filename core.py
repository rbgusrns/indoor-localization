...
def simulate_static_position(base_pos=(10, 3.5), jitter=0.3):
    x = base_pos[0] + random.uniform(-jitter, jitter)
    y = base_pos[1] + random.uniform(-jitter, jitter)
    return (x, y)
...
def on_detect(rssi_vec, mv, fp, ekf, k, speed, yaw):
    ...
    if len(on_detect.rssi) >= 4: #BLE 스캔으로 받은 RSSI가 일정 개수 이상일 때만
        ...
        pts = np.array(pts)
        print(f"RSSI: {on_detect.rssi} BLE: (7,2) ")
        #print(f"BLE: {pts}")
        ...
        fused_pos = simulate_static_position()
